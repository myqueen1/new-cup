...
//    放入回收站
    public function delAll()
    {
        ...
        $data['goods_status'] = 3;
        if (M('goods_detailed')->where($where)->save($data)) {
...
//    回收站列表
    public function recycle_bin()
    {
        ...
        $count = M('goods')->join("five_goods_detailed ON five_goods.goods_id = five_goods_detailed.goods_id")->where($map)->count();// 查询满足要求的总记录数
    ...
    public function del()
    {
        ...
        if (M('goods')->where($where)->delete()) {
            ...
            $this->redirect('Goods/recycle_bin', "", 0, '页面跳转中...');
        } else {
            $this->error("删除失败");
        }
